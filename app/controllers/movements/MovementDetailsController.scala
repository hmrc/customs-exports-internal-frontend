...
class MovementDetailsController @Inject()(
  ...
  private def handleSavingArrival(arrivalAnswers: ArrivalAnswers)(implicit request: JourneyRequest[AnyContent]): Future[Either[Html, Call]] = {
    ...
    details
      ...
      .bindFromRequest()
      .fold(
        (formWithErrors: Form[ArrivalDetails]) => Future.successful(Left(arrivalDetailsPage(withDateSpecificErrors(formWithErrors), arrivalAnswers.consignmentReferences))),
        validForm =>
  ...
  private def handleSavingDeparture(departureAnswers: DepartureAnswers)(implicit request: JourneyRequest[AnyContent]): Future[Either[Html, Call]] = {
    ...
    details
      ...
      .bindFromRequest()
      .fold(
        (formWithErrors: Form[DepartureDetails]) => Future.successful(Left(departureDetailsPage(withDateSpecificErrors(formWithErrors), consignmentReference))),
        validForm =>
