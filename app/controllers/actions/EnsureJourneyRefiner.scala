...
class EnsureJourneyRefiner @Inject()(movementRepository: MovementRepository)(implicit val exc: ExecutionContext) {

  def journey(types: JourneyType*): ActionRefiner[AuthenticatedRequest, JourneyRequest] = new ActionRefiner[AuthenticatedRequest, JourneyRequest] {
    override protected def refine[A](request: AuthenticatedRequest[A]): Future[Either[Result, JourneyRequest[A]]] =
      movementRepository.findByPid(request.operator.pid).map(_.map(_.answers)).map {
        case Some(answers: Answers) if types.contains(answers.`type`) => Right(JourneyRequest(answers, request))
