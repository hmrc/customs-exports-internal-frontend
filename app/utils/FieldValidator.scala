...
object FieldValidator {
  ...
  val containsDuplicates: Iterable[?] => Boolean = (input: Iterable[?]) => input.toSet.size != input.size

  val areAllElementsUnique: Iterable[?] => Boolean = (input: Iterable[?]) => input.toSet.size == input.size
