  ...
  "Choice page page" should {
    ...
    "render back link and point to appropriate page" when {
      "form contains ucr block" in {
        val backButton = page(form, Some(UcrBlock(ucr = "ucr", ucrType = Ducr.codeValue))).getElementById("back-link")
    ...
    "not render 'Shut Mucr' option" when {
      "ILE query was for a Ducr" in {
        val choicePage = page(form, Some(UcrBlock(ucr = "ducr", ucrType = Ducr.codeValue)))
      ...
      "ILE query was for a Ducr part" in {
        val choicePage = page(form, Some(UcrBlock(ucr = "ducr-part", ucrType = DucrPart.codeValue)))
    ...
    "render 'Shut Mucr' and 'Retrospective Arrival' options" when {
      "ILE query was for a Mucr" in {
        val choicePage = page(form, Some(UcrBlock(ucr = "mucr", ucrType = Mucr.codeValue)))
