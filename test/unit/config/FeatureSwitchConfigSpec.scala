...
class FeatureSwitchConfigSpec extends UnitSpec {
  ...
  private val validConfig: Config = ConfigFactory.parseString("features.default=disabled".stripMargin)
  ...
  "Feature Switch config" should {
    ...
    "throw an exception when features.default is missing" in {
      intercept[Exception](emptyFeatureSwitchConfig.defaultFeatureStatus).getMessage mustBe
        "Missing configuration key: features.default"
    }
  }
}
