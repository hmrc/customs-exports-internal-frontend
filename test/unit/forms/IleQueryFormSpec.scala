...
class IleQueryFormSpec extends UnitSpec {
  ...
  "IleQueryForm" should {
    ...
    "convert ducr to upper case" in {
      val form = IleQueryForm.form.bind(Json.obj("isIleQuery" -> "cds", "ucr" -> " 8gb123457359100-test0001 "), JsonBindMaxChars)
      form.errors mustBe empty
      form.value must be(Some(Some("8GB123457359100-TEST0001")))
    }

    "convert mucr to upper case" in {
      val form = IleQueryForm.form.bind(Json.obj("isIleQuery" -> "cds", "ucr" -> " gb/abced1234-15804test "), JsonBindMaxChars)
      form.errors mustBe empty
      form.value must be(Some(Some("GB/ABCED1234-15804TEST")))
    }

    "convert mucr to upper case when is 35 characters long" in {
      val form = IleQueryForm.form.bind(Json.obj("isIleQuery" -> "cds", "ucr" -> " gb/82f9-0n2f6500040010tO120p0a30998 "), JsonBindMaxChars)
      form.errors mustBe empty
      form.value must be(Some(Some("GB/82F9-0N2F6500040010TO120P0A30998")))
    }

    "display error when mucr is over 35 characters long" in {
      val form = IleQueryForm.form.bind(Json.obj("isIleQuery" -> "cds", "ucr" -> "gb/82f9-0n2f6500040010tO120p0a309989"), JsonBindMaxChars)
